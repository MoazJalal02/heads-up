    ...
    return (
        <>
            <button className={styles.button} onClick={() => toggleDropdown()}>
                { !session? (
                  <svg className='account--icon' width="24" height="32" viewBox="0 0 36 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <Image
                    src={session.user.image ?? '/assets/caps-images/1.png'} 
                    alt="" 
                    sizes="100vw"
                    width={24}
                    height={24}
            ...
            {session? (
                    <div className={styles.dropdownContent} ref={dropdownRef} style={{ display: `${isDropdownOpen ? 'block' : 'none'}` }}>
                      {session.user?.role == 'Admin'? <Link href="/admin/add-product">Admin</Link>:''}
